...
exports.onAuditRunCreated = void 0;
...
exports.onAuditRunCreated = (0, firestore_1.onDocumentCreated)({
    ...
    try {
        const result = await (0, enqueue_audit_task_js_1.enqueueAuditTask)(payload, {
            project: PROJECT,
            location: REGION,
            queue: QUEUE,
        ...
        const mode = isDev ? 'direct-worker' : 'cloud-tasks';
        await persistEnqueueMode(snapshot.ref, mode, runId);
        log('info', result.deduped
            ? 'Audit task already enqueued — dedupe via deterministic name'
            : 'Enqueued audit task', { runId, taskName: result.taskName, deduped: result.deduped, mode });
    }
    catch (err) {
...
function buildPayload(runId, data) {
...
//# sourceMappingURL=on-audit-run-created.js.map