...
exports.__resetClientForTests = __resetClientForTests;
exports.enqueueAuditTask = enqueueAuditTask;
...
function getClient() {
    if (!cachedClient)
        cachedClient = new tasks_1.CloudTasksClient();
    return cachedClient;
}
/** Test-only: reset the cached client (lets mocks replace the SDK). */
function __resetClientForTests(client) {
    cachedClient = client;
}
/**
 ...
 */
async function postDirectlyToWorker(payload, workerUrl) {
...
function isEmulatorOrDev() {
    return process.env.FUNCTIONS_EMULATOR === 'true';
}
async function enqueueAuditTask(payload, config) {
 ...
 */
function emitMetric(event, runId, taskName) {
...
//# sourceMappingURL=enqueue-audit-task.js.map