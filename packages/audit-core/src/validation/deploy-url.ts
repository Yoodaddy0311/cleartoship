...
//
// Shared by both the web app (POST /api/audit-runs) and the audit worker
// (VALIDATE_INPUT pipeline step) so SSRF defense is identical on both sides
// of the Cloud Tasks queue hop (defense-in-depth: the worker re-validates
// because the queue payload is not authoritative).
//
